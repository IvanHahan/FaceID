...
LOCAL_ROOT = '/home/ihahanov/Projects/FaceID/'
...
def read_image(image_path):
    ...
    if not os.path.exists(image_path[:-4] + '_BB.txt'):
        return img
...
def get_image(max_number=None):
    ...
    """
    with open('/home/ihahanov/Projects/FaceID/data/test_spoofed.txt') as s, \
        open('/home/ihahanov/Projects/FaceID/data/test_live1.txt') as l:
        spoofed_list_dir = s.readlines()
        live_list_dir = l.readlines()
    spoof_list = []
    for i in spoofed_list_dir:
        spoof_list.extend(glob.glob(os.path.join(LOCAL_ROOT, i.strip(), '*')))

    live_list = []
    for i in live_list_dir:
        live_list.extend(glob.glob(os.path.join(LOCAL_ROOT, i, '*')))

    label = [*np.zeros(len(spoof_list)), *np.ones(len(live_list))]
    image_list = [*spoof_list, *live_list]
    ...
    final_image = []
    final_image_id = []
    for idx, (image_id, l) in enumerate(zip(image_list, label)):
        ...
        if n == Batch_size or idx == len(image_list) - 1:
            ...
            final_image = []
            final_image_id = []
            yield np_final_image_id, np_final_image, l
...
def verify_output(output_probs):
    ...
    scores = []
    labels = []
    for k, label in output_probs.values():
    ...
    print(sum(np.asarray(np.array(scores) > 0.5).astype('int') == labels) / len(labels))
